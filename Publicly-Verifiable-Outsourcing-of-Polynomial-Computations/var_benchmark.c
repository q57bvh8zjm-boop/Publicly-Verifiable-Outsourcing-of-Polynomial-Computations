...
// Exploratory application-level estimator using atomic operations from the scheme implementations
#include "scheme1.h"
#include "scheme2.h"
#include "scheme3.h"
#include "sodium.h"
...
int main() {
    ...
    // ==========================================
    // Scheme 1 and Scheme 2 use F_q2 field operations.
    // ==========================================
    ...
    // ==========================================
    // Scheme 3 uses Paillier-based HSS.
    // ==========================================
    paillier_pk_t pk;
    ...
    printf("Running Scheme 3/HSS issuance (sampled %d encryptions to avoid 16h runtime)...\n", n_sample_hss);
    start = clock();
    ...
    printf("Running Scheme 3/HSS server audit for %lld users...\n", n_users);
    start = clock();
    for (int i = 0; i < n_users; i++) {
        // Paillier addition
    ...
    printf("Running Scheme 3/HSS verification...\n");
    hss_mv_t mv0, mv1;
    ...
    printf("| S-VAR (Paper, k=1) | 0.89 s | 34 s | < 100 us |\n");
    printf("| Scheme 1/2 | %.4f s | %.4f s | %.4f ms |\n", t_s1_issue, t_s1_server, t_s1_verify * 1000.0);
    printf("| Scheme 3/HSS | %.4f s* | %.4f s | %.4f ms |\n", t_s3_issue, t_s3_server, t_s3_verify * 1000.0);

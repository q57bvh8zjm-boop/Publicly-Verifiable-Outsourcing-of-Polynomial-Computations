...
void prf_zz(fmpz_t res, int *prf_key, fmpz_t modulus) {
    ...
    // Used by FLINT? No, we convert manually
    unsigned char out[64]; // Enough for N (assuming < 512 bits? No, N is 3072 bits!)
...
int scheme3_verify(scheme3_verify_out *out, unsigned char **vk_x,
    ...
    for(int i = 0; i <= d; i++) {
        ...
        printf("DEBUG: i=%d\n", i);
        printf("  f_i = "); fq_print_pretty(f_values[i], par->Fp); printf("\n");
        printf("  lambda'_i = "); fq_print_pretty(lambda_prime[i], par->Fp); printf("\n");
        printf("  exponent = "); fq_print_pretty(exponents[i], par->Fp); printf("\n");
    }
...
void scheme3_compute_lambda_prime(fq_t *lambda_prime, fq_t *v_values, int d, pubpar *par) {
    ...
    for(int j = 0; j <= d; j++) {
        ...
        // ：Product_{i,k in {0,...,d}, i<k} (v_k - v_i)
        for(int i = 0; i <= d; i++) {
            for(int k = i + 1; k <= d; k++) {

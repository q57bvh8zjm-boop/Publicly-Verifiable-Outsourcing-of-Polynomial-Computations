...
int main() {
    ...
    start = clock();
    for (int i = 0; i < iterations; i++) {
        crypto_scalarmult_ristretto255(vk, scalar, point);
    }
    end = clock();
    ...
    for (int t = 1; t <= 5; t++) {
        ...
        for (int d = 1; d <= 20; d++) {
            ...
            // Server cost: For degree d, server might need (d-1) * n multiplications
            // Plus scalar multiplications which are faster.
            // We assume server scans n items.
            double server_time_s = (double)n_db * (d-1) * t_hss_mul; 
            double server_time_h = server_time_s / 3600.0;
